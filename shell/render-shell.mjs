...
function applyInteractivePlaceholders(bodyHtml, project) {
  ...
  const scope =
    ...
    "Fixed demo corpus only; production pilots scope your own documents, auth, and hosting.";
  return bodyHtml
...
async function main() {
  ...
  bodyHtml = applyRagUrls(bodyHtml, ragPublicUrl, ragDocsUrl, ragAskUrl);
  ...
  const footerSub =
    ...
    "Retrieval + generation · FastAPI · FAISS";
  const railHomeUrl =
    profile.railHomeUrl || profile.portfolioUrl || "https://github.com/vahdetkaratas";
  ...
  if (demoBodyArg) {
    ...
    demoBody = applyRagUrls(demoBody, ragPublicUrl, ragDocsUrl, ragAskUrl);
