...
3. **CORS (if the site calls the API from the browser):** Set `CORS_ORIGINS` to a comma-separated list of **exact frontend origins** (no trailing slash), e.g. `https://vahdetkaratas.com,https://www.vahdetkaratas.com`. If you leave it empty, the API allows `*` (any origin). **If you deploy with only `http://localhost:…` in `CORS_ORIGINS`, your production portfolio page will get “Failed to fetch” in the browser.**
