...
3. **CORS (if the site calls the API from the browser):** Set `CORS_ORIGINS` to a comma-separated list of **exact frontend origins** (no trailing slash). Include **every** host the UI runs on—subdomains differ from the apex domain (e.g. portfolio on `https://vahdetkaratas.com` vs RAG demo shell on `https://rag.vahdetkaratas.com` calling API `https://rag-qa.vahdetkaratas.com`). Example: `https://vahdetkaratas.com,https://www.vahdetkaratas.com,https://rag.vahdetkaratas.com`. If you leave it empty, the API allows `*` (any origin). **If you deploy with only `http://localhost:…` in `CORS_ORIGINS`, your production pages will get “Failed to fetch” in the browser.**
