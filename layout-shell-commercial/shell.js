...
(function () {
  ...
  function askUrl() {
    ...
    if (isLocal) return window.location.origin + "/ask";

    if (fromMeta) {
      try {
        if (new URL(fromMeta).hostname.toLowerCase() === host) {
  ...
  function fillContextDetails(chunks) {
    ...
    ctxMeta.textContent = parts.length
      ? "First retrieved row: " + parts.join(" | ")
      : "First retrieved row (API payload).";
  ...
  function buildSourceCard(s) {
    ...
    art.setAttribute("aria-label", "Evidence: " + (s.document || "-"));
    var title = document.createElement("div");
    title.className = "proj-source-doc";
    title.textContent = s.document || "-";
    ...
    row("Page", s.page != null ? String(s.page) : "-");
    row("Chunk ID", s.chunk_id || "-");
  ...
  askBtn.addEventListener("click", function (e) {
    ...
    fetch(askUrl(), {
      ...
      .then(function (r) {
        ...
        if (!r.ok) {
          var detail =
            r.data && (r.data.detail !== undefined ? r.data.detail : r.data);
          statusEl.textContent =
            "HTTP " + r.status + (detail ? ": " + JSON.stringify(detail) : "");
          statusEl.classList.add("is-error");
          return;
        }
